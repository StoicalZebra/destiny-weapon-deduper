<template>
  <header class="bg-surface-elevated border-b border-border">
    <div class="container mx-auto px-4 py-4 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-6">
        ...
        <div v-if="authStore.isAuthenticated" class="flex items-center gap-3">
          ...
          <div class="relative group">
            <div
              class="w-8 h-8 rounded-full bg-surface-overlay flex items-center justify-center cursor-default hover:bg-border transition-colors"
              :title="displayName"
            >
              <!-- Xbox -->
              <svg v-if="platformType === 1" class="w-5 h-5 text-green-600 dark:text-green-400" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- PlayStation -->
              <svg v-else-if="platformType === 2" class="w-5 h-5 text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="currentColor">
              ...
              <!-- Steam (default for 3+) -->
              <svg v-else class="w-5 h-5 text-text-muted" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="absolute right-0 top-full mt-2 px-3 py-1.5 bg-surface text-text text-sm rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50 border border-border">
              {{ displayName }}
            </div>
          </div>
          <button
            @click="logout"
            class="text-sm text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300 transition"
          >
            Logout
          </button>
        </div>
      ...
      <div class="flex md:hidden items-center gap-2">
        ...
        <button
          ...
          class="p-2 text-text-muted hover:text-text hover:bg-surface-overlay rounded-lg transition-colors"
    ...
    <div
      v-if="mobileMenuOpen"
      class="md:hidden border-t border-border bg-surface-elevated"
    >
          ...
          <div v-if="authStore.isAuthenticated" class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div class="w-8 h-8 rounded-full bg-surface-overlay flex items-center justify-center">
                ...
                <svg v-else-if="platformType === 2" class="w-5 h-5 text-blue-600 dark:text-blue-400" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M8.985 2.596v17.548l3.915 1.261V6.688c0-.69.304-1.151.794-.991.636.181.76.814.76 1.505v5.875c2.441 1.193 4.362-.002 4.362-3.153 0-3.237-1.126-4.675-4.438-5.827-1.307-.448-3.728-1.186-5.393-1.501z"/>
                </svg>
                <svg v-else class="w-5 h-5 text-text-muted" viewBox="0 0 24 24" fill="currentColor">
